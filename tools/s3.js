  ...
  clearBucket () {
    return new Promise((resolve, reject) => {
      this.client.listObjects({ Bucket: this.bucket }, (err, data) => {
        if (err) {
          reject(new Error(`error listing bucket objects ${err}`))
          return
        }
        ...
        items.forEach((item, index) => {
          ...
          this.client.deleteObject(deleteParams, (err) => {
            if (err) {
              reject(new Error(`delete err ${deleteParams.Key}`))
            }
    ...
    return new Promise((resolve, reject) => {
      ...
      fs.readdir(folderPath, (err, files) => {
        if (!files || files.length === 0) {
          reject(new Error(`provided folder '${distPath}' is empty or does not exist.`))
          return
        }
